...
from api.api_auth_user import get_current_active_user, check_admin_rights
...
def update_notification(notification_id: int, notification_sch: schema.NotificationUpdate, 
                        db: Session = Depends(get_db),
                        current_user: schema_user.User=Depends(check_admin_rights)):
     db_notification = crud.get_notification_by_id(db, notification_id)
     if not db_notification:
